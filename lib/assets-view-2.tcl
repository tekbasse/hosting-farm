...
if { $item_count > 0 } {
    ...
    # as_label as_name as_type metric latest_sample percent_quota projected_eop score score_message
    ns_log Notice "assets-view-2.tcl(45): table_cols_count $table_cols_count table_index_last $table_index_last "
    ...
    if { $s_exists_p && $s ne "" } {
        ...
        ns_log Notice "hosting-farm/lib/assets-view-2.tcl(73): sort_order_scalar $sort_order_scalar"
    ...
    ns_log Notice "assets-view.tcl.149"
    ...
    if { $p_exists_p && $p ne "" } {
        ...
        ns_log Notice "hosting-farm/lib/assets-view-2.tcl(85): primary_sort_col_new $primary_sort_col_new"
        if { $primary_sort_col_new ne "" && $primary_sort_col_pos < $table_cols_count } {
            # ns_log Notice "hosting-farm/lib/assets-view-2.tcl(87): primary_sort_col_new $primary_sort_col_new primary_sort_col_pos $primary_sort_col_pos"
            ...
            foreach ii $sort_order_list {
                if { [expr { abs($ii) } ] ne $primary_sort_col_pos } {
                    ...
                    # ns_log Notice "hosting-farm/lib/assets-view-2.tcl(93): ii '$ii' sort_order_new_list '$sort_order_new_list'"
                }
            }
            set sort_order_list $sort_order_new_list
            # ns_log Notice "hosting-farm/lib/assets-view-2.tcl(97): end if primary_sort_col_new.. "
        }
    }
    ns_log Notice "assets-view.tcl.175"
    if { ( $s_exists_p && $s ne "" ) || ( $p_exists_p && $p ne "" ) } {
        # ns_log Notice "hosting-farm/lib/assets-view-2.tcl(101): sort_order_scalar '$sort_order_scalar' sort_order_list '$sort_order_list'"
        ...
        ns_log Notice "hosting-farm/lib/assets-view-2.tcl(104): sort_rev_order_list '$sort_rev_order_list' "
        foreach ii $sort_rev_order_list {
            set col2sort [lindex $sort_order_list $ii]
            # ns_log Notice "hosting-farm/lib/assets-view-2.tcl(107): ii $ii col2sort '$col2sort' llength col2sort [llength $col2sort] sort_rev_order_list '$sort_rev_order_list' sort_order_list '$sort_order_list'"
            ...
            if {[catch { set table_sorted_lists [lsort $sort_type -dictionary $sort_order -index $col2sort_wo_sign $table_sorted_lists] } result]} {
                ...
                ns_log Notice "hosting-farm/lib/assets-view-2.tcl(121): lsort fell back to sort_type -ascii due to error: $result"
            }
            #ns_log Notice "hosting-farm/lib/assets-view-2.tcl(123): lsort $sort_type $sort_order -index $col2sort_wo_sign table_sorted_lists"
        }
    }
    ...
    ns_log Notice "assets-view.tcl.309"
    ...
    ns_log Notice "assets-view.tcl.349"
    foreach title $table_titles_list {
        ...
        ns_log Notice "assets-view.tcl.375"

        if { $primary_sort_col eq "" || ( $primary_sort_col ne "" && $column_count ne [expr { abs($primary_sort_col) } ] ) } {
            if { $column_sorted_p } {
                ns_log Notice "hosting-farm/lib/assets-view-2.tcl(150): column_count $column_count s_urlcoded '$s_urlcoded'"
                if { $decreasing_p } {
            ...
            if { $decreasing_p } {
                # ns_log Notice "hosting-farm/lib/assets-view-2.tcl(154): column_count $column_count title $title s_urlcoded '$s_urlcoded'"
                ...
                # ns_log Notice "hosting-farm/lib/assets-view-2.tcl(158): column_count $column_count title $title s_urlcoded '$s_urlcoded'"
    ...
    ns_log Notice "assets-view.tcl.490"
    ...
    foreach table_row_list $table_sorted_lists {
        ...
        if { $table_row_new_cols != $table_cols_count } {
            ns_log Notice "hosting-farm/lib/assets-view-2.tcl(203): Warning: table_row_new has ${table_row_new_cols} instead of ${table_cols_count} columns."
        }
    ...
    foreach td_row_list $cell_table_lists {
        ...
        ns_log Notice "assets-view.tcl.633"
        # Now that the sorted columns are added to the row, add the remaining columns
        foreach ui $unsorted_list {
            if { $ui ne "" } {
