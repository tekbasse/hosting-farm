...
ad_library {
    ...
    Asset utility library for Hosting Farm
...
ad_proc -private hf_asset_ids_for_user { 
    ...
    If top_level_p is "1", returns only the top level assets ie not assets dependent on other assets.
} {
    upvar 1 instance_id instance_id
    set asset_ids_list [list ]
    if { [qf_is_natural_number $user_id] } {
        set pkg_admin_p [permission::permission_p -party_id $user_id -object_id [ad_conn package_id] -privilege admin]
        if { $pkg_admin_p } {
...
ad_proc -private hf_asset_type_id_of_asset_id {
    ...
    if { !$exists_p } {
        ns_log Notice "hf_asset_type_id_of_asset_id: asset_id '${asset_id}' does not exist. instance_id '${instance_id}'"
    }
...
ad_proc -private hf_asset_id_change {
    ...
    if { $write_p } {
        ...
        db_dml hf_asset_id_change { update hf_asset_rev_map
            set asset_id=:asset_id_new where label=:label and instance_id=:instance_id }
        hf_asset_trash $asset_id
...
ad_proc -private hf_asset_cascade_count {
    ...
    while { $current_id_list_len > 0 && $not_error_p } {
        ...
        incr count
        if { [expr { int( $count / 1000. ) } ] == [expr { $count / 1000. } ] } {
            ns_log Notice "hf_asset_cascade_count.545: count '${count}'"
        }
...
ad_proc -private hf_asset_subassets_by_type_cascade {
    ...
    while { $current_id_list_len > 0 && $not_error_p } {
        ...
        incr count
        if { [expr { int( $count / 1000. ) } ] == [expr { $count / 1000. } ] } {
            ns_log Notice "hf_asset_subassets_by_type_cascade.586: count '${count}'"
        }
...
ad_proc -private hf_asset_attributes_by_type_cascade {
    ...
    while { $current_id_list_len > 0 && $not_error_p } {
        ...
        incr count
        if { [expr { int( $count / 1000. ) } ] == [expr { $count / 1000. } ] } {
            ns_log Notice "hf_asset_attributes_by_type_cascade.669: count '${count}'"
        }
