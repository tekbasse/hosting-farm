...
ad_proc -public hfdt_vh_attr_create {
    f_id
} {
    part of test suite and demo.
    Creates a base virtual host as attribute only, no asset,
    and a dependent user account.
    Requires connecting to existing attribute or asset.
} {
    # These are for audits in test cases. See hosting-farm-test-api-procs.tcl
...
ad_proc -public hfdt_ss_base_create {
    {f_id ""}
} {
    part of test suite and demo.
...
ad_proc -public hfdt_ss_attr_create {
    f_id
} {
    part of test suite and demo.
    Creates a base virtual host as attribute only, no asset,
...
ad_proc -public hfdt_ua_asset_create {
    f_id
} {
    part of test suite and demo.
    Creates a user account as an asset.
    Requires connecting to existing attribute or asset.
} {
    # These are for audits in test cases. See hosting-farm-test-api-procs.tcl
    upvar 1 audit_ac_arr audit_ac_arr
    ...
    if { $head_f_id ne "" } {
        ...
        if { $head_type_id ne "" } {
            ...
            if { $ua_arr(f_id) > 0 } {
                ...
                if { $head_type_id ne "" } {
                    # map to existing asset
                    hf_sub_asset_map_update $head_f_id $head_type_id $ua_arr(label) $ua_arr(f_id) $asset_type_id 0
                }
...
ad_proc -public hfdt_hw_base_create {
    ...
    if { $hw_arr(f_id) > 0 } {
        ...
        if { $hw_arr(hw_id) ne "" } {
            ...
            ns_log Warning "hfdt_hw_base_create.956: hf_hw_write failed"
        }
...
ad_proc -public hfdt_hw_1u_create {
    ...
    if { $hw_arr(f_id) > 0 } {
        ...
        if { $hw_arr(hw_id) ne "" } {
            ...
            ns_log Warning "hfdt_hw_1u_create: hf_hw_write failed"
        }
