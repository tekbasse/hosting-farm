...
ad_library {
    ...
    misc API for hosting-farm attributes
...
ad_proc -private hf_dc_hw_ids {
    dc_id
    instance_id
} {
    Returns list of hardware ids associated with a dc_id, or empty list if there are none.
} {
    set hw_id_list [db_list hf_dc_hw_ids_get "select hw_id from hf_dc_hw_map where instance_id=:instance_id and dc_id=:dc_id"]
    return $hw_id_list
}

ad_proc -private hf_dc_ni_ids {
    dc_id
    instance_id
} {
    Returns list of network interface ids associated with a dc_id, or empty list if there are none.
} {
    set ni_id_list [db_list hf_dc_ni_ids_get "select ni_id from hf_dc_ni_map where instance_id=:instance_id and dc_id=:dc_id"]
    return $ni_id_list
}


ad_proc -private hf_hw_ni_ids {
    hw_id
    instance_id
} {
    Returns list of network interface ids associated with a hw_id, or empty list if there are none.
} {
    set ni_id_list [db_list hf_hw_ni_ids_get "select ni_id from hf_hw_ni_map where instance_id=:instance_id and hw_id=:hw_id"]
    return $ni_id_list
}

ad_proc -private hf_asset_ip_ids {
    asset_id
    instance_id
} {
    Returns list of ip_id directly associated with an asset, or empty list if there are none.
} {
    set ip_id_list [db_list hf_asset_ip_ids_get "select ip_id from hf_asset_ip_map where asset_id=:asset_id and instance_id=:instance_id"]
    return $ip_id_list
}
...
# These procs return data in list of list "table" format about an asset given one or more asset ids.
